...
# データベースに保存
items.each do |item|
  Rails.logger.debug { "Creating product: #{item[:name]}" }
  ...
  if response.success?
    ...
    Rails.logger.debug { "#{item[:name]} created!" }
  else
    Rails.logger.debug { "Error fetching image for #{item[:name]}: #{response.status}" }
  end
rescue StandardError => e
  Rails.logger.debug { "Error creating product for #{item[:name]}: #{e.message}" }
end
